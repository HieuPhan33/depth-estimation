...
class ResNetSkipConcat(nn.Module):
    def __init__(self, layers, output_size, decoder='nnconv5dw', in_channels=3, pretrained=True):
        ...
        #     self.decode_conv5 = upsample(128, 32,pooling_output_size=(48,48))
            ...
            self.decode_conv2 = nn.Sequential(ChannelwiseLocalAttention(pooling_output_size=(4,4),n_heads=2),upsample(768, 256))
            self.decode_conv3 = nn.Sequential(ChannelwiseLocalAttention(pooling_output_size=(8,8),n_heads=4),upsample(384, 128))
            self.decode_conv4 = nn.Sequential(ChannelwiseLocalAttention(pooling_output_size=(16,16),n_heads=8),upsample(192, 64))
            self.decode_conv5 = nn.Sequential(ChannelwiseLocalAttention(pooling_output_size=(32,32),n_heads=16),upsample(128, 32))
            #self.decode_conv2 = nn.Sequential(ChannelwiseLocalAttention(pooling_output_size=(4,4),r=2),upsample(768, 256))
            #self.decode_conv3 = nn.Sequential(ChannelwiseLocalAttention(pooling_output_size=(8,8),r=4),upsample(384, 128))
            #self.decode_conv4 = nn.Sequential(ChannelwiseLocalAttention(pooling_output_size=(16,16),r=8),upsample(192, 64))
            #self.decode_conv5 = nn.Sequential(ChannelwiseLocalAttention(pooling_output_size=(32,32),r=16),upsample(128, 32))
        # self.decode_conv1 = conv(1024, 512, kernel_size)
