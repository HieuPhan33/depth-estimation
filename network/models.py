...
class ResNetSkipConcat(nn.Module):
    def __init__(self, layers, output_size, decoder='nnconv5dw', in_channels=3, pretrained=True):
        ...
        #     self.decode_conv5 = upsample(128, 32,pooling_output_size=(48,48))
            ...
            self.decode_conv2 = nn.Sequential(Seq_Ex_Block(768,4),
                                              upsample(768, 256))
            self.decode_conv3 = nn.Sequential(Seq_Ex_Block(384,4),
                                              upsample(384, 128))
            self.decode_conv4 = nn.Sequential(Seq_Ex_Block(192,4),
                                              upsample(192, 64))
            self.decode_conv5 = nn.Sequential(Seq_Ex_Block(128,4),
                                              upsample(128, 32))
        # self.decode_conv1 = conv(1024, 512, kernel_size)
